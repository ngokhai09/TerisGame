...
public class Lr_Block implements Shape{
	...
	public String toString() {
		return "KhoiHinhLr [h1=" + h1 + ", h2=" + h2 + ", h3=" + h3 + ", h4=" + h4 + "]";
	}
	@Override
			...
			else
			{
				if (x==h2.getDiem().getX())
				{
					if (y>h2.getDiem().getY())
					{
						if (h3.touchl(game))
						{
							if (game[j][i+1].isRong()&&game[j][i+2].isRong())
							{
								h2.move(x, y);
								h1.move(x, y-size);
								h3.move(x+size, y);
	...
	public boolean touchl(Square[][] game) {
		if (h2.getDiem().getX()==h3.getDiem().getX())
		{
			if (h3.getDiem().getY()>h2.getDiem().getY())
			{
				if (h2.touchl(game)||h3.touchl(game)||h4.touchl(game))
				{
					return true;
				}
				else
	...
	public boolean touchr(Square[][] game) {
		if (h2.getDiem().getX()==h3.getDiem().getX())
		{
			...
			{
				if (h2.touchr(game)||h3.touchr(game)||h4.touchr(game))
				{
					return true;
				}
				else
		...
		else
		{
			...
			{
				if (h1.touchr(game)||h2.touchr(game))
				{
					return true;
				}
				else
